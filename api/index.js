...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (url.startsWith('/api/admin/count')) {
      if (!ADMIN_TOKEN) return res.status(401).json({error: 'Admin API token not configured'});
      const r = await fetch('https://' + SHOPIFY_DOMAIN + '/admin/api/' + API_VERSION + '/products/count.json', {headers: {'X-Shopify-Access-Token': ADMIN_TOKEN}});
      return res.json(await r.json());
    }
